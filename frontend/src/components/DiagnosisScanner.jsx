...
export default function DiagnosisScanner({ onGetTreatment, language = 'en', isOffline }) {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col items-center justify-center animate-in fade-in duration-700 p-4 sm:p-6 pb-12 overflow-x-hidden">
      ...
      <div className="w-full max-w-6xl z-10 pt-4">
        ...
        {/* Upload Box */}
        <div className="bg-white/80 backdrop-blur-xl border border-white rounded-[2rem] sm:rounded-[2.5rem] p-2 sm:p-4 shadow-[0_20px_60px_rgba(43,36,25,0.05)] w-full">
          ...
          {!file && (
            <label
              ...
              onDrop={handleDrop}
              className={`relative flex flex-col items-center justify-center p-6 sm:p-12 rounded-[1.75rem] sm:rounded-[2.25rem] border-2 border-dashed transition-all duration-500 overflow-hidden min-h-[350px] sm:min-h-[400px] w-full max-w-4xl mx-auto ${
                isDragging 
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-24 sm:py-32 rounded-[2.25rem] w-full">
          ...
          {error && !loading && (
            <div className="flex flex-col items-center justify-center py-20 sm:py-24 px-6 sm:px-8 rounded-[2.25rem] w-full">
          ...
          {result && !loading && !error && (
            <div className="p-4 sm:p-6 lg:p-8 animate-in zoom-in-95 duration-500 rounded-[2.25rem] w-full">
              {/* Flex layout with constrained image width to give content more room */}
              <div className="flex flex-col lg:flex-row gap-6 sm:gap-8 lg:items-stretch">
                
                {/* Image Preview - Fixed optimal width on desktop */}
                <div className="w-full lg:w-[320px] xl:w-[380px] shrink-0 relative rounded-2xl sm:rounded-3xl overflow-hidden border border-soil-ink/10 shadow-[0_10px_30px_rgba(33,150,243,0.15)] min-h-[250px] sm:min-h-[350px]">
                  <img src={file} alt="Analyzed Crop" className="absolute inset-0 w-full h-full object-cover" />
                ...
                {/* Analysis Data - Takes up remaining space */}
                <div className="flex-1 flex flex-col justify-between w-full">
                  
                  {/* Status Banner */}
                  <div className={`p-4 sm:p-6 mb-6 rounded-2xl sm:rounded-3xl flex items-center space-x-4 sm:space-x-5 border backdrop-blur-sm shadow-sm ${
                    result.type === 'pest'
                    ...
                    <div>
                      ...
                      <p className="text-soil-ink/70 text-xs sm:text-sm font-sans flex items-center flex-wrap gap-1.5">
                        <span className="bg-white/60 px-1.5 sm:px-2 py-0.5 rounded-md font-bold text-soil-ink shadow-sm">{result.confidence}%</span>
                        <span>{language === 'hi' ? 'AI विश्वास मिलान' : 'AI Confidence Match'}</span>
                      </p>
                    </div>
                  </div>
                  
                  {/* Actionable Cards - Flex wrap prevents squishing */}
                  <div className="flex flex-wrap gap-4 sm:gap-5 flex-1">
                    
                    {/* Treatment Card */}
                    <div className="flex-1 min-w-[240px] bg-gradient-to-b from-well-water-blue/5 to-transparent rounded-2xl sm:rounded-3xl p-5 border border-well-water-blue/10 relative overflow-hidden group flex flex-col">
                      ...
                      <div className="flex items-center justify-between mb-3 relative z-10">
                        <h4 className="font-sans font-bold text-soil-ink flex items-center text-sm sm:text-md">
                          <div className="p-1.5 bg-white rounded-xl shadow-sm mr-2.5 shrink-0">
                      ...
                      <p className="text-soil-ink/80 font-sans text-sm leading-relaxed relative z-10 flex-1">
                        {result.treatment}
                      </p>
                    </div>
                    
                    {/* Prevention Card */}
                    <div className="flex-1 min-w-[240px] bg-gradient-to-b from-leaf-green/5 to-transparent rounded-2xl sm:rounded-3xl p-5 border border-leaf-green/10 relative overflow-hidden group flex flex-col">
                      ...
                      <h4 className="font-sans font-bold text-soil-ink mb-3 flex items-center text-sm sm:text-md relative z-10">
                        <div className="p-1.5 bg-white rounded-xl shadow-sm mr-2.5 shrink-0">
                    ...
                    {/* Fertilizer Card */}
                    <div className="flex-1 min-w-[240px] bg-gradient-to-b from-amber-500/5 to-transparent rounded-2xl sm:rounded-3xl p-5 border border-amber-500/10 relative overflow-hidden group flex flex-col">
                  ...
                  {/* Buttons */}
                  <div className="mt-6 sm:mt-8 flex flex-row flex-wrap justify-center lg:justify-end gap-3 sm:gap-4 w-full">
                    {onGetTreatment && result.name !== 'Healthy' && (
                      <button 
                        onClick={() => onGetTreatment(result.name)}
                        className="flex-1 sm:flex-none group relative inline-flex items-center justify-center space-x-2 sm:space-x-3 bg-[#1E1911] text-husk-cream px-5 sm:px-8 py-3 sm:py-3.5 rounded-full font-bold shadow-[0_8px_20px_rgba(43,36,25,0.2)] hover:shadow-[0_12px_25px_rgba(43,36,25,0.3)] hover:-translate-y-1 transition-all duration-300 overflow-hidden min-w-[200px]"
                      >
                        <div className="absolute inset-0 bg-white/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out"></div>
                        <Sparkles className="relative z-10 w-4 h-4 sm:w-5 sm:h-5 shrink-0" />
                        <span className="relative z-10 text-sm sm:text-base whitespace-nowrap">{language === 'hi' ? 'AI उपचार योजना' : 'Get AI Treatment Plan'}</span>
                      </button>
                    )}
                    <button 
                    ...
                    >
                      <div className="absolute inset-0 bg-white/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out"></div>
                      <span className="relative z-10 text-sm sm:text-base whitespace-nowrap">Scan another leaf</span>
                      <UploadCloud className="relative z-10 group-hover:-translate-y-1 transition-transform duration-300 w-4 h-4 sm:w-5 sm:h-5 shrink-0" />
                    </button>
                  </div>
                </div>
              </div>
